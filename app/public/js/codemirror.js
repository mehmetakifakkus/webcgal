window.CodeMirror = function () {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        for (var opt in defaults)
            if (defaults.hasOwnProperty(opt)) options[opt] = (givenOptions && givenOptions.hasOwnProperty(opt) ? givenOptions : defaults)[opt];
        ...
        var shiftSelecting, lastClick, lastDoubleClick, lastScrollTop = 0,
            ...
            suppressEdits = false,
            pasteIncoming = false;
        var updateInput, userSelChange, changes, textChanged, selectionChanged, gutterDirty, callbacks;
        var displayOffset = 0,
            showingFrom = 0,
            showingTo = 0,
            lastSizeC = 0;
        ...
        var instance = wrapper.CodeMirror = {
            ...
            refresh: function () {
                ...
                if (scrollbar.scrollHeight > lastScrollTop) scrollbar.scrollTop = lastScrollTop
            },
        ...
        function lineContent(line, wrapAt) {
            if (!line.styles) line.highlight(mode, line.stateAfter = getStateBefore(lineNo(line)), options.tabSize);
        ...
        function onScrollMain(e) {
            if (options.fixedGutter && gutter.style.left != scroller.scrollLeft + "px") gutter.style.left = scroller.scrollLeft + "px";
            if (Math.abs(scroller.scrollTop - lastScrollTop) > 1) {
                lastScrollTop = scroller.scrollTop;
                if (scrollbar.scrollTop != lastScrollTop) scrollbar.scrollTop = lastScrollTop;
                updateDisplay([])
            }
        ...
        function onDragStart(e) {
            ...
            if (e.dataTransfer.setDragImage) e.dataTransfer.setDragImage(elt("img"), 0, 0)
        }
        ...
        function onFocus() {
            ...
            if (!focused) {
                ...
                focused = true;
                if (scroller.className.search(/\bCodeMirror-focused\b/) == -1) scroller.className += " CodeMirror-focused"
            }
            slowPoll();
            restartBlink()
        }
        ...
        function updateLinesNoUndo(from, to, lines, selFrom, selTo) {
            ...
            if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == "") {
                var added = [],
                    prevLine = null;
                for (var i = 0, e = lines.length - 1; i < e; ++i) added.push(new Line(hlText(lines[i]), hlSpans(lines[i])));
                ...
                if (lines.length == 1) {
                    ...
                    for (var added = [], i = 1, e = lines.length - 1; i < e; ++i) added.push(new Line(hlText(lines[i]), hlSpans(lines[i])));
                ...
                for (var i = 1, e = lines.length - 1; i < e; ++i) added.push(new Line(hlText(lines[i]), hlSpans(lines[i])));
        ...
        function replaceRange(code, from, to) {
            ...
            function adjustPos(pos) {
                ...
                var ch = pos.ch;
                if (pos.line == to.line) ch += lst(code).length - (to.ch - (to.line == from.line ? from.ch : 0));
                return {
                    line: line,
                    ch: ch
                }
        ...
        function readInput() {
            ...
            else if (overwrite && posEq(sel.from, sel.to) && !pasteIncoming) sel.to = {
                line: sel.to.line,
        ...
        function focusInput() {
            if (options.readOnly != "nocursor" && (ie_lt9 || document.activeElement != input)) input.focus()
        }
        ...
        function updateDisplay(changes, suppressCallback, scrollTop) {
            ...
            if (from != showingFrom || to != showingTo && options.onViewportChange) setTimeout(function () {
        ...
        function updateSelection() {
            ...
            if (!collapsed) {
                ...
                var add = function (left, top, right, height) {
                    var rstyle = quirksMode ? "width: " + (!right ? clientWidth : clientWidth - right - left) + "px" : "right: " + (right - 1) + "px";
        ...
        function indentLine(n, how) {
            ...
            if (indentString != curSpaceString) replaceRange(indentString, {
                line: n,
                ch: 0
            }, {
                line: n,
        ...
        function findMarksAt(pos) {
            ...
            if (spans)
                for (var i = 0; i < spans.length; ++i) {
                    var span = spans[i];
                    if ((span.from == null || span.from <= pos.ch) && (span.to == null || span.to >= pos.ch)) markers.push(span.marker)
                }
            return markers
        }
            ...
            function rehide() {
                ...
                if (input.selectionStart != null) {
                    ...
                    detectingSelectAll = setTimeout(function poll() {
                        if (prevInput == " " && input.selectionStart == 0) operation(commands.selectAll)(instance);
        ...
        function highlightWorker() {
            ...
            if (showingTo > startFrontier && frontier >= showingFrom) operation(function () {
                changes.push({
                    from: startFrontier,
                    to: frontier
                })
        ...
        function endOperation() {
            ...
            if (changes.length || newScrollPos && newScrollPos.scrollTop != null) updated = updateDisplay(changes, true, newScrollPos && newScrollPos.scrollTop);
            if (!updated) {
            ...
            if (focused && (updateInput === true || updateInput !== false && selectionChanged)) resetInput(userSelChange);
            ...
            var sc = selectionChanged,
                cbs = callbacks;
            if (textChanged && options.onChange && instance) options.onChange(instance, textChanged);
        ...
        for (var ext in extensions)
            if (extensions.propertyIsEnumerable(ext) && !instance.propertyIsEnumerable(ext)) instance[ext] = extensions[ext];
    ...
    CodeMirror.resolveMode = function (spec) {
        ...
        else if (typeof spec == "string" && /^[\w\-]+\/[\w\-]+\+xml$/.test(spec)) return CodeMirror.resolveMode("application/xml");
    ...
    function updateMarkedSpans(oldFirst, oldLast, startCh, endCh, newText) {
        ...
        if (!sameLine) {
            ...
            for (var i = 0; i < gap; ++i) newMarkers.push(newHL(newText[i + 1], gapMarkers));
    ...
    function attachMarkedSpans(line, spans) {
        if (!spans) return;
        for (var i = 0; i < spans.length; ++i) var marker = spans[i].marker.lines.push(line);
    ...
    Line.prototype = {
        ...
        process: function (mode, state, tabSize) {
            var stream = new StringStream(this.text, tabSize);
            if (this.text == "" && mode.blankLine) mode.blankLine(state);
            while (!stream.eol() && stream.pos <= 5e3) {
...
CodeMirror.defineMode("javascript", function (config, parserConfig) {
    ...
    return {
        ...
        indent: function (state, textAfter) {
            ...
            else if (type == "stat") return lexical.indented + (state.lastType == "operator" || state.lastType == "," ? indentUnit : 0);
